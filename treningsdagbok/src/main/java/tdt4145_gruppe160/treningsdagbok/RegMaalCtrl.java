...
package tdt4145_gruppe160.treningsdagbok;
/**
 *
 * @author sveinbra
 */
...
public class RegMaalCtrl extends DBConn {
    ...
    public void regPost (int sekvNr, int postNr, int tid) {
        if (brikkeNr != INGEN_BRIKKE) {
            try {
                ...
                System.out.println("suksess reg");
            } catch (Exception e) {
    ...
    public boolean sluttReg () {
    	...
    	try {
            ...
            ResultSet rs = stmt.executeQuery("select sekvnr, postnr, tid from Reg");
            while (rs.next()) {
            	System.out.println(rs.getInt("sekvnr"));
                /*if (startTid == -1) {
    ...
    public void test() {
    	try {
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery("select klassenavn, lnr from Klasse");
            while (rs.next()) {
            	System.out.println(rs.getString("klassenavn")+" "+rs.getInt("lnr"));
                /*if (startTid == -1) {
                    startTid = rs.getInt("tid");
                }
                sluttTid = rs.getInt("tid");
    	...
    	try {
            Statement statement = conn.createStatement();
            statement.executeUpdate("INSERT INTO Klasse " + "VALUES ('H51', 55)");
            System.out.println("suksess klasse");
        } catch (Exception e) {
            System.out.println("db error during insert of Klasse");
        }
    	//Oppdatert
    	try {
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery("select klassenavn, lnr from Klasse");
            while (rs.next()) {
            	System.out.println("Oppdatert "+rs.getString("klassenavn")+" "+rs.getInt("lnr"));
                /*if (startTid == -1) {
                    startTid = rs.getInt("tid");
                }
                sluttTid = rs.getInt("tid");
